...
if (isset($_GET["q"])) {
	$queryTerm = '%'.$_GET['q'].'%';
	$result=$mysqli->query("SELECT * FROM colleges WHERE name LIKE '$queryTerm' LIMIT 8");
	while($row = $result->fetch_assoc())
	{
		$college['label'] = $row['name'];
	...
	$college['label'] = "Sorry no matches found";
	$college['value'] = "Sorry no matches found";
	$college['id'] = 0;
	$data[] = $college;
}
